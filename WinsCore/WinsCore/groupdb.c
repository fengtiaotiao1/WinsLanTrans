...
int addGroupMemberData(PGroupInfo pTransInfo) {
    ...
    if (DB_SUCCESS == ret) {
        if (0 == iNum) {
            ...
            printf("insert database table has this data\n");
        }
    }
...
int deleteGroupMemberData(PGroupInfo pData) {
    ...
    if (DB_SUCCESS != iQRet || iNum == 0) {
        printf("deleteGroupMemberData query trans tbl iNum is : %d\n", iNum);
        return DB_FAILED;
    }
...
int deleteMemberDataByName(const char *memberName) {
    ...
    printf("deleteMemberDataByName query trans tbl iNum is : %d\n", iNum);
    char regular[1024] = {'\0'};
    // 拼接sql语句
...
int deleteGroupDataById(int groupId) {
    ...
    if (DB_SUCCESS != iQRet || iNum == 0) {
        printf("deleteGroupDataById query trans tbl iNum is : %d\n", iNum);
        return DB_FAILED;
    }
...
int updateMemberDataByName(const char *memberName, const char *newIp) {
    ...
    if (DB_SUCCESS == ret) {
        if (iNum > 0) {
            ...
            printf("update database table name num is 0!\n");
        }
    }
    return DB_FAILED;
}
